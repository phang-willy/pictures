...
function parseCountryListItems(data: unknown): ApiCountry[] {
  ...
  for (const item of rawItems) {
    ...
    const iso2 = typeof candidate.iso2 === "string" ? candidate.iso2 : null;
    if (!id || !name || !iso2) {
      continue;
    }
    const slug =
      typeof candidate.slug === "string" && candidate.slug.trim().length > 0
        ? candidate.slug
        : countrySlugFallbackFromName(name);
    out.push({
      id,
      name,
      iso2,
  ...
  useEffect(() => {
    ...
    const loadCitiesForCountry = async (
      ...
      try {
        ...
        const cities = parseCityListPayload(json);
        if (
          isDisposed ||
          mapRef.current !== map ||
    ...
    void fetch(apiUrl("/api/country?geometry=true&activate=true"))
      .then(async (response) => {
        if (isDisposed || mapRef.current !== map) {
          return;
        }
