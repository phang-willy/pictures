...
export function CountryNewForm({
  ...
  const [codeIso3, setCodeIso3] = useState("");
    ...
    const t = window.setTimeout(() => {
      ...
      if (!hit) {
        ...
        const identityEmpty =
          !name.trim() &&
          !slug.trim() &&
          codeIso2.trim().length < 2 &&
          codeIso3.trim().length < 3;
        if (identityEmpty) {
  ...
  }, [name, codeIso2, codeIso3, slug, searchDriver, mledozeList]);
  ...
  useEffect(() => {
      ...
      void (async () => {
        ...
        try {
          const q = new URLSearchParams({
            ...
            codeIso3: codeIso3.trim().toUpperCase(),
            name: name.trim(),
    ...
    return () => window.clearTimeout(t);
  }, [canCheckExists, name, codeIso2, codeIso3, slug]);
  ...
  useEffect(() => {
    const iso3 = codeIso3.trim().toUpperCase();
    if (iso3.length !== 3) {
      worldGeometrySeededForIso3Ref.current = null;
      setGeoHint(null);
      return;
    ...
    if (worldGeometrySeededForIso3Ref.current === iso3) {
      return;
    }
    let cancelled = false;
    void (async () => {
      try {
        ...
        const feature = (payload.features ?? []).find(
          (f) => String(f.id ?? "").toUpperCase() === iso3,
        );
        if (
          ...
          const countryLabel =
            name.trim() || englishCommonReference?.trim() || iso3;
          setGeoHint(
        ...
        worldGeometrySeededForIso3Ref.current = iso3;
        setGeomType(gType);
  ...
  }, [codeIso3, existsResult, loadWorldGeo, name, englishCommonReference]);
    ...
    <form onSubmit={onSubmit} className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          {mledozeError ||
          existsFetchError ||
          (canCheckExists && !existsLoading && existsResult?.exists) ? (
                  ...
                  <AlertDescription>
                    {existsResult.match ? (
                      <>
                        <p>
                          <strong>
                            {existsResult.match.name} (
                            {existsResult.match.codeIso2}{" "}
                            {existsResult.match.codeIso3
                              ? ` / ${existsResult.match.codeIso3}`
                              : ""}
                            )
                          </strong>
