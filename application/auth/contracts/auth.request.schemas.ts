import { z } from "zod";
import { emailSchema, otpCodeSchema, passwordSchema } from "./auth.primitives";
...
/** Jeton signe (email + date d'expiration) renvoye apres login, a renvoyer avec le code OTP. */
export const verifyTwoFactorRequestSchema = z.object({
  twoFactorToken: z.string().min(1, 'Session de connexion invalide ou expiree.'),
  code: otpCodeSchema,
});
...
export const forgotPasswordResetRequestSchema = z
  .object({
    token: z.string().min(1, 'Lien de reinitialisation invalide.'),
    email: emailSchema,
    password: passwordSchema,
